...
(* let chaines_croissantes l =
    let rec chaines_croissantes_rec l acc1 acc2 acc3 =
        match l with
        [] -> acc1
        |h::t -> if h >= acc2 then chaines_croissantes_rec t acc1 h (List.append acc3 h)
                 else chaines_croissantes_rec t (List.append acc1 [acc3]) 0::[] []
        in
        chaines_croissantes_rec l [[]] 0::[] []
;; *)
